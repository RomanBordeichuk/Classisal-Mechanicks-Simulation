...
export default class AfterCollDirectionCalculator{
    static calculateDirection(collision: Collision){
        if(!(collision.obj1 instanceof Circle)) throw new Error("Incorrect object type");
        if(!(collision.obj2 instanceof Circle)) throw new Error("Incorrect object type");
    ...
    private static _calcMirroredAngle(collFlatAngle: number, obj: Circle){
        if(obj.pinned == true) return;

        let angleDelta = obj.direction - collFlatAngle;
        if(angleDelta < -90) angleDelta += 180;
        else if(angleDelta > 90) angleDelta -= 180;
        ...
        if(vx < 0){
            if(vy > 0) newDirection = 180 + newDirection;
            else newDirection = 180 - newDirection;
        }
