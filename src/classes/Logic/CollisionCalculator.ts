...
export default class CollisionCalculator{
    static calculateFirstCollisionOrUnderfined(objList: Structure[], dt: number): Collision | undefined{
        let collisionsList: Collision[] = [];

        objList.forEach(obj1 => {
            if(!(obj1 instanceof Circle)) throw new Error ("Incorrect object type");

            if(obj1.pinned == false){
                objList.forEach(obj2 => {
                    if(!(obj2 instanceof Circle)) throw new Error ("Incorrect object type");
        ...
        let firstCollision: Collision | undefined;

        collisionsList.forEach(coll => {
            if(firstCollision === undefined) firstCollision = coll;
            else if(coll.t < firstCollision.t) firstCollision = coll;
        });
